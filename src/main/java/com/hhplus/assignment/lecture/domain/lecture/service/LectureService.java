...
public class LectureService {
    ...
    private final LectureEnrollHistoryRepositoryImpl lectureEnrollRepositoryImpl;
    private final OpenCourseRepositoryImpl openCourseRepositoryImpl;
...
//        LectureDto lecture = lectureRepositoryImpl.findById(request.getLectureId());
        ...
        OpenCourseDto course = openCourseRepositoryImpl.findById(request.getCourseId());
        // case 2. 수강신청 인원이 30명 이상인 경우
        // 해당 lectureId에 해당하는 특강의 수강신청 인원이 30명 이상일 경우, 수강신청 인원이 초과되었다는 에러 메시지를 반환한다.
        if (openCourseRepositoryImpl.isFull(course.getMaxStudentCount(), course.getCapacity())) {
        ...
        if (lectureEnrollRepositoryImpl.isAlreadyEnrolled(userId, course.getCourseId())) {
            log.error("이미 수강신청한 특강입니다.");
            throw new LectureException(HttpStatus.BAD_REQUEST, "ALREADY_ENROLLED", "이미 수강신청한 강의입니다.");
        }

        // 수강신청 인원을 1명 증가시킨다.
        openCourseRepositoryImpl.increaseEnrollStudentCount(course.getCourseId());

        // 특강 신청정보를 저장한다.
        LectureEnrollHistoryDto result = lectureEnrollRepositoryImpl.enrollCourse(userId, course.getCourseId());
    ...
    public LectureResponse<List<OpenCourseDto>> openCourseList(LectureOpenCourseSearchRequestParam request) {
        ...
        return new LectureResponse<>(openCourseRepositoryImpl.findAllById(request.getLectureId()), "특강 목록 조회가 완료되었습니다.");
    }
    ...
    public LectureResponse<LectureEnrollHistoryDto> checkEnrolledCourse(long userId, CheckEnrollCourseRequestParam param) {
        ...
        return new LectureResponse<>(lectureEnrollRepositoryImpl.findByEnrollmentId(LectureEnrollmentId.builder()
                .userId(userId)
